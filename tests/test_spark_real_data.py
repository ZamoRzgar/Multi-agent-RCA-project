"""
Test RCA system on real Spark data from loghub.

This script tests the complete multi-agent RCA system on actual
Spark logs with real failure scenarios.
"""
...
def test_spark_scenario(scenario_id: int = 1):
    """
    Test RCA system on a single Spark scenario.
    
    Args:
        scenario_id: Which scenario to test (1-5)
    """
    print_section("Spark REAL DATA TEST")
    print(f"Testing Scenario {scenario_id}")
    
    # Step 1: Load Spark data
    print_section("STEP 1: LOADING Spark DATA")
    ...
    # Save results
    output_file = f"spark_scenario_{scenario_id}_results.json"
...
def test_multiple_scenarios():
    """Test on multiple Spark scenarios."""
    print_section("TESTING MULTIPLE Spark SCENARIOS")
    ...
    for scenario_id in range(1, 4):  # Test first 3 scenarios
        ...
        try:
            results = test_spark_scenario(scenario_id)
            all_results.append({
                "scenario_id": scenario_id,
                "success": True,
    ...
    print("\n" + "="*70)
    print("✓ Spark REAL DATA TESTING COMPLETE!")
    print("="*70)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        test_spark_scenario(scenario_id)
    else:
