"""
Test RCA system on real Hadoop data from loghub.

This script tests the complete multi-agent RCA system on actual
Hadoop logs with real failure scenarios.
"""
...
def test_hadoop_scenario(scenario_id: int = 1):
    """
    Test RCA system on a single Hadoop scenario.
    
    Args:
        scenario_id: Which scenario to test (1-5)
    """
    print_section("Hadoop REAL DATA TEST")
    print(f"Testing Scenario {scenario_id}")
    
    # Step 1: Load Hadoop data
    print_section("STEP 1: LOADING Hadoop DATA")
    ...
    # Save results
    output_file = f"hadoop_scenario_{scenario_id}_results.json"
...
def test_multiple_scenarios():
    """Test on multiple Hadoop scenarios."""
    print_section("TESTING MULTIPLE Hadoop SCENARIOS")
    ...
    for scenario_id in range(1, 4):  # Test first 3 scenarios
        ...
        try:
            results = test_hadoop_scenario(scenario_id)
            all_results.append({
                "scenario_id": scenario_id,
                "success": True,
    ...
    print("\n" + "="*70)
    print("✓ Hadoop REAL DATA TESTING COMPLETE!")
    print("="*70)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        test_hadoop_scenario(scenario_id)
    else:
