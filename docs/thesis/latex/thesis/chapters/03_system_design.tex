...
\begin{tikzpicture}[node distance=0.6cm, font=\small,
    layer/.style={rectangle, draw=black, thick, minimum width=11cm, minimum height=1cm, align=center},
    component/.style={rectangle, draw=blue!60, fill=blue!10, thick, minimum width=1.6cm, minimum height=0.6cm, align=center, rounded corners=2pt, font=\footnotesize}
]
% Layers (bottom to top)
\node[layer, fill=gray!15] (infra) at (0,0) {\textbf{Infrastructure}: Ollama (LLM) + Neo4j (Graph DB)};
\node[layer, fill=yellow!10] (knowledge) at (0,1.3) {\textbf{Knowledge}: Incident KG + Similarity Retrieval};
\node[layer, fill=blue!8] (agent) at (0,2.6) {\textbf{Agent}: Parser, KG Retrieval, 3 Reasoners, Judge};
\node[layer, fill=orange!10] (orch) at (0,3.9) {\textbf{Orchestration}: Debate Protocol + Workflow Control};
\node[layer, fill=green!8] (pres) at (0,5.2) {\textbf{Presentation}: Evaluation Scripts + Result Collection};
...
\end{tikzpicture}
\caption{Layered architecture of \systemname{}. Each layer has well-defined responsibilities and interfaces with adjacent layers through structured data contracts.}
...
\begin{tikzpicture}[node distance=0.8cm, font=\small,
    box/.style={draw, rounded corners, align=center, minimum width=3.2cm, minimum height=0.7cm}
]
  ...
  \node[box, fill=green!10, below left=0.6cm and -1.5cm of kg] (logr) {Log Reasoner};
  \node[box, fill=green!10, below=0.6cm of kg] (hyb) {Hybrid Reasoner};
  \node[box, fill=green!10, below right=0.6cm and -1.5cm of kg] (kgr) {KG Reasoner};
  \node[box, fill=orange!15, below=0.6cm of hyb] (judge) {Judge Agent};
  ...
  \draw[->, dashed, thick, red!60] (judge.east) -- ++(1.2,0) -- ++(0,2.4) -- (kgr.east);
  \node[font=\tiny, red!60, anchor=west] at (4.2,-3.5) {Refinement};
\end{tikzpicture}
...
The system runs for up to 3 rounds. In each round, reasoners generate hypotheses; the judge scores them and provides feedback; and reasoners refine hypotheses based on feedback and other agents' best hypotheses. The debate stops when scores plateau or the maximum rounds are reached. Figure~\ref{fig:debate} illustrates the debate flow.

\begin{figure}[H]
\centering
\begin{tikzpicture}[node distance=0.7cm, font=\small,
    process/.style={rectangle, draw=blue!60, fill=blue!10, thick, minimum width=2.2cm, minimum height=0.7cm, align=center, rounded corners=3pt},
    decision/.style={diamond, draw=orange!60, fill=orange!10, thick, minimum width=1.2cm, align=center, aspect=2.5, font=\footnotesize},
    data/.style={rectangle, draw=green!60, fill=green!10, thick, minimum width=1.8cm, minimum height=0.6cm, align=center, rounded corners=2pt}
]
% Input
...
% Output
\node[data, fill=green!25] (output) at (3,-7.5) {Final Hypothesis};

% Refinement
\node[process, fill=purple!10, draw=purple!60, minimum width=1.6cm] (refine) at (-3,-7.5) {Refine};
...
\draw[->] (refine) -- (-3,-1.5);
\end{tikzpicture}
\caption{Multi-agent debate protocol flow. Three reasoners generate hypotheses in parallel; the judge evaluates and provides feedback; refinement continues until convergence or max rounds (3).}
\label{fig:debate}
\end{figure}
