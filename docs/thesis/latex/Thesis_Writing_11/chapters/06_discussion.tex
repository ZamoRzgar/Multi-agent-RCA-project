...
The ablation study in Section~\ref{sec:ablation} confirms that each architectural component
contributes measurably to overall performance. Removing the judge produces the largest
single-component drop ($-$14.4\,pp macro-\fone{} on CMCC), validating that the verification
mechanism is the primary driver of correctness. Removing KG retrieval reduces macro-\fone{}
by 7.8\,pp, confirming the value of historical grounding---particularly for failure classes
with strong recurrence patterns in the knowledge graph. Reducing debate from two rounds to
one costs 6.3\,pp, consistent with multi-agent debate literature showing that most gain
accrues in the first refinement round with diminishing returns
thereafter~\cite{du2023improvingfactualityreasoninglanguage}. The component ordering
(Judge~$>$~KG~$>$~Debate) is preserved on Hadoop1, suggesting it reflects a structural
property of the architecture rather than a dataset-specific artifact. These results provide
clean attribution of performance gains to specific architectural choices and support the
design decision to invest additional LLM calls in judging and retrieval over additional
debate rounds.
...
compute cost that may be prohibitive for strict real-time incident response. Fourth, while we include a supervised logistic regression baseline and contextualize
our results against published deep learning numbers
(Table~\ref{tab:hdfs-dl-comparison})~\cite{du2017deeplog,guo2021logbert,zhang2019logrobust,ijcai2019p658},
we do not retrain these DL models ourselves. The literature comparison confirms the expected
accuracy gap between supervised methods (93--99\% \fone{}) and our zero-shot approach (69.4\%),
but a controlled re-evaluation under identical splits and metrics remains future work.
