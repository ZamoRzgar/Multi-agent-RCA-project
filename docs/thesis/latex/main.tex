...
The current evaluation compares only against a single-agent LLM baseline. While this demonstrates the value of multi-agent debate, stronger baselines would provide more context:

\begin{itemize}
    \item \textbf{Chain-of-Thought (CoT)}: Single LLM with explicit reasoning steps
    \item \textbf{Retrieval-Augmented Generation (RAG)}: Single LLM with retrieved examples
...
These are acknowledged as future work due to time constraints.
